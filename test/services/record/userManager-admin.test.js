...
module.exports = function(chai, app) {
  ...
  it('Cant Update record - regular user to RegularUserTwo', function(done) {
    chai.request(app)
      .patch('/api/records/' + config.admin.record_id)
      .set('Accept', 'application/json')
      .set('Authorization', 'Bearer ' + token)
      .send({
        "time": "00:50:00",
  ...
  it('Cant Delete record - regular user to RegularUserTwo', function(done) {
    chai.request(app)
      .delete('/api/records/?id=' + config.admin.record_id)
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (res.body.code != 403) {
          should.fail();
  ...
  it('Cant Get list of records - regular user to RegularUserTwo', function(done) {
    chai.request(app)
      .get('/api/records?from=1489269600000&to=1491944340000&userId=' + config.admin.uuid)
      .set('Accept', 'application/json')
      .set('Authorization', 'Bearer ' + token)
      .send(user)
      .end((err, res) => {
